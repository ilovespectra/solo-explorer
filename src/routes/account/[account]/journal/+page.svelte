...
<script lang="ts">
    ...
    $: sortedEntries = journalEntries.sort(
    ...
    const fetchJournalEntries = async () => {
        ...
        try {
            const querySnapshot = await getDocs(
                query(
                    collection(db, "FreewriteEntries"),
...
<div class="container mb-3 lowercase">
    ...
    {:else}
        ...
        {#if isSuccessMessageVisible}
            <p>
                <i>your freewrite journal entry has been logged successfully!</i
                >
            </p>
        {/if}

        <div class="journal-entries container lowercase">
            {#if sortedEntries.length > 0}
                {#each sortedEntries as entry}
                    <div class="journal-entry-item">
                        ...
                        <button
                            class="delete-button"
                            on:click={() => removeJournalEntry(entry.id)}
                            >X</button
                        >
                    </div>
                {/each}
