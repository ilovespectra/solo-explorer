...
<Stats />
<nav class="sticky left-0 top-0 z-40 grid h-full grid-cols-6 items-center justify-between border bg-black p-1 px-0">
    <div class="col-span-4 flex items-center md:col-span-2">
        <div class="mx-2 flex items-center" in:fly={{ duration: 750, x: -50 }}>
            <a class="btn btn-ghost px-2" href="/" rel="noreferrer">
    ...
    <div class="col-span-2 flex items-center justify-end">
        <div class="flex justify-end pr-2">
            <button class="btn btn-ghost" on:click={() => showModal("HELP")}>
                <Icon id="question" size="md" />
            </button>
            <button class="btn btn-ghost" on:click={() => showModal("MENU")}>
...
{#if $page.url.pathname !== "/"}
    <button class="btn btn-secondary btn-sm fixed bottom-4 right-3 z-10 cursor-pointer rounded-full" on:click={() => window.scrollTo({ behavior: "smooth", top: 0 })}>
